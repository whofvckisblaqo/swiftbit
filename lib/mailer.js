...
export async function sendKycStatusEmail(email, name, status) {
  ...
  resend.emails.send({
    from: FROM,
    to: email,
    subject: approved ? 'Your identity has been verified — SwiftBit' : 'KYC application update — SwiftBit',
    html: baseTemplate(`
      <p style="color:#9ca3af;font-size:14px;margin:0 0 24px">Hi ${name},</p>
      <h2 style="font-size:22px;font-weight:800;margin:0 0 6px">
        ${approved ? 'Identity Verified ✓' : 'KYC Application Rejected'}
      </h2>
      <p style="color:#9ca3af;font-size:14px;margin:0 0 28px">
        ${approved
      ...
      <div style="background:${approved ? 'rgba(74,222,128,0.06)' : 'rgba(248,113,113,0.06)'};border:1px solid ${approved ? 'rgba(74,222,128,0.2)' : 'rgba(248,113,113,0.2)'};border-radius:12px;padding:20px;margin-bottom:28px;text-align:center">
        <span style="font-size:32px">${approved ? '🎉' : '⚠️'}</span>
        <p style="color:${approved ? '#4ade80' : '#f87171'};font-weight:700;font-size:15px;margin:10px 0 0">
