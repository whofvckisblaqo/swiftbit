...
function sendMail(payload) {
  return resend.emails.send(payload).then(r => {
    if (r.error) console.error('[mailer] Resend error:', r.error, '| from:', payload.from, '| to:', payload.to);
    else console.log('[mailer] sent:', payload.subject, '→', payload.to);
    return r;
  }).catch(err => console.error('[mailer] send failed:', err.message, '| key set:', !!process.env.RESEND_API_KEY));
}
